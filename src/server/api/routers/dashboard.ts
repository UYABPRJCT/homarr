...
const getDashboardById = async (id: string, prisma: PrismaClient) => {
...
export const dashboardRouter = createTRPCRouter({
  byId: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const widgetOptions = await ctx.prisma.widgetOption.findMany({
        where: {
          widget: {
            item: {
              groupId: {
  ...
  save: publicProcedure.input(dashboardSchema).mutation(async ({ ctx, input }) => {
    const dashboard = await getDashboardById(input.id, ctx.prisma);
